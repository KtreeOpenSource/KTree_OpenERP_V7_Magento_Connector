...
class magento_configuration(osv.osv):
    ...
    def export_customers(self,cr,uid):
		...
		categ=cus['category_id'] and cus['category_id'][0].magento_id or False
                customer_data = {
                                 'email'      : cus['email'],
                                 'group_id'   : cus['group_id'].magento_id or categ,
...
#            server.endSession(session)
	...
	while(True):
	  ...
	  if last_import:
            customers = server.call(session, 'customer.list',[ {'updated_at': {'from': last_import}}])
          else:  
	  ...
	  index = stop
         ...
         #Fetching Product,Based on min and max number from magneto (Due to Response error)
          ...
          while length_cust>0:
		    ...
		    cust_grp_ids=[]
                    if info_customer.has_key('group_id') and int(info_customer['group_id']):
                    ...
                    erp_customer = {  
                                    ...
                                    'group_id'       : cust_grp_ids and cust_grp_ids[0] or False,
                    ...
                    if cust_ids == []:
                      if 1==1:
                        ...
                        for cus_add in customer_address:
                            ...
                            try:
                                state_id = self.pool.get('res.country.state').name_search(cr, uid, cus_add['region'])[0][0]
                            except:
                                if 'region' in cus_add.keys():
                                        new_state = { 
                                                        'name': cus_add['region'],
                                                        'code': cus_add['region'],
                                                        'country_id' : country_id,
                                                       }
                                        state_id = self.pool.get('res.country.state').create(cr, uid, new_state)
                            phone=''
...
#                                self.pool.get('res.partner').write(cr, uid, [line], {'parent_id':False,'magento_id':-2} )        
                cr.commit()
	  if last_import or not customers:
		break
...
class res_partner(osv.osv):
	...
	'created_at': lambda *a: fields.date.today(),
    }
